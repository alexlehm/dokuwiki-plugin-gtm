...
class action_plugin_googletagmanager extends DokuWiki_Action_Plugin {
	...
	function _addHeaders (&$event, $param) {
		global $INFO;
		if(!$this->getConf('GTMID')) return;
		$event->data['noscript'][] = array (
		  '_data' => '<iframe src="//www.googletagmanager.com/ns.html?id='.$this->getConf('GTMID').'" height="0" width="0" style="display:none;visibility:hidden"></iframe>',
		);
...
})(window,document,'script','dataLayer','".$this->getConf('GTMID')."');",
		);
	}
}
?>